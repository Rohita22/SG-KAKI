...
const SECTION_META: Record<Category, { title: string; description: string }> = {
  food: { title: '🍜 Food', description: 'Hawker orders, kopi lingo, and what to say at the table.' },
  lingo: { title: '💬 Lingo', description: 'Everyday Singlish that gets you talking like a local.' },
  socialVibes: { title: '🎭 Culture', description: 'Unspoken rules, etiquette, and how to read the room.' },
  gettingAround: { title: '🚇 Transport', description: 'MRT, buses, and getting around the island.' },
  workCulture: { title: '💼 Work', description: 'Office norms and how work actually gets done here.' },
};
  ...
  return (
    <div className="relative">
      ...
      <input
        ...
        className="w-full rounded-2xl border border-white/60 bg-white/70 py-3 pl-11 pr-4 text-sm font-semibold text-sg-navy shadow-sm backdrop-blur-sm outline-none placeholder:text-sg-navy/35 focus:border-sg-blue/40 focus:ring-4 focus:ring-sg-blue/15"
      />
    </div>
  );
}
...
function PhrasesTab({ query, filter }: { query: string; filter: Filter }) {
  ...
  const matchesFilter = (p: Phrase) => {
    if (query && !matchesQuery(p.word, query) && !matchesQuery(p.meaning, query)) return false;
  ...
  const showRecentSection = filter === 'all' && !query && recentPhrases.length > 0;
  const visibleCategories =
    filter === 'all' || filter === 'favorites' || filter === 'recent' ? categories : [filter as Category];
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
            {recentPhrases.map((p, i) => (
              <div key={p.id} className="relative">
                {i === 0 && (
                  <span className="absolute -top-2 left-1/2 z-10 -translate-x-1/2 rounded-full bg-sg-success px-2 py-0.5 text-[9px] font-black text-white shadow-card">
                    Latest
                  </span>
                )}
                <PhraseGridCard
      ...
      {visibleCategories.map((category) => {
        ...
        return (
          <Card key={category} className="border border-white/50 bg-white/70 backdrop-blur-md">
            <p className="text-sm font-black text-sg-navy">{SECTION_META[category].title}</p>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
              {phrasesInCategory.map((p) => {
                ...
                const mission = activeCountryPack.missions.find((m) => m.id === p.missionId);
                return (
                  <PhraseGridCard
                    key={p.id}
                    icon={CATEGORY_META[p.category].emoji}
...
function CultureTab({ query, filter }: { query: string; filter: Filter }) {
  ...
  const topics = activeCountryPack.cultureTopics.filter((t) => {
    if (query && !matchesQuery(t.title, query) && !matchesQuery(t.summary, query)) return false;
      ...
      <Card className="border border-white/50 bg-white/70 backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">
          {topics.map((topic) => {
            ...
            const mission = activeCountryPack.missions.find((m) => m.id === topic.missionId);
            return (
              <PhraseGridCard
                key={topic.id}
                icon={topic.icon}
        ...
        {selected && (
          <div className="flex flex-col gap-4 rounded-3xl bg-white p-6 shadow-card-lg">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-extrabold text-sg-navy">{selected.title}</h2>
            </div>
            ...
            <AskSGBuddy
              context={{
                missionTitle:
                  activeCountryPack.missions.find((m) => m.id === selected.missionId)?.title ?? '',
  ...
  return (
    <div className="relative -mx-4 -my-5 min-h-full px-4 py-5 sm:-mx-6 sm:px-6 lg:-mx-10 lg:-my-8 lg:px-10 lg:py-8">
      <div
        ...
        style={{ backgroundImage: `url(${FIELD_GUIDE_BG_URL})` }}
        aria-hidden="true"
      />
      <div className="pointer-events-none absolute inset-0 z-0 bg-gradient-to-b from-white/50 via-white/25 to-white/60" aria-hidden="true" />

      <div className="relative z-10 flex flex-col gap-5">
        <FieldGuideHero />

        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="inline-flex w-fit gap-1 rounded-2xl bg-white/60 p-1 backdrop-blur-sm">
            <button
              type="button"
              onClick={() => setTab('phrases')}
              className={clsx(
                'rounded-xl px-4 py-2 text-sm font-bold transition-colors',
                tab === 'phrases' ? 'bg-white text-sg-navy shadow-card' : 'text-sg-navy/50',
              )}
            >
              📖 Phrases
            </button>
            <button
              ...
              className={clsx(
                'rounded-xl px-4 py-2 text-sm font-bold transition-colors',
                tab === 'culture' ? 'bg-white text-sg-navy shadow-card' : 'text-sg-navy/50',
              )}
            >
              🎓 Culture Notes
            </button>
